...
/*
 * NOTE: This version of MurmurHash3 was modified to avoid misaligned access of
 * 32-bit values.
 *
 * Instead of separating the key into blocks followed by a tail like so:
 *
 *   [block 1] [block 2] [block n] [tail]
 *
 * align the blocks and use the unaligned head instead:
 *
...
{
  const uint8_t * head = (const uint8_t*)(key);
  const int padding = ((uintptr_t)4 - (uintptr_t)key) % 4;
  const int head_len = len > padding ? padding : len;
  const int block_len = len - head_len;
  const int nblocks = block_len / 4;
  const uint32_t * blocks = (const uint32_t *)(head + head_len);
  ...
  switch(head_len)
  {
  case 3: k1 ^= head[2] << 16; /* fall through */
  case 2: k1 ^= head[1] << 8; /* fall through */
  case 1: k1 ^= head[0]; /* fall through */
