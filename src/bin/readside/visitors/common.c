				...
				const struct side_attr **attr, u32 *nr_attr)
{
	...
	mappings = visit_side_sel_pointer(ctx, type->mappings);
			...
			const struct side_attr **attr, u32 *nr_attr)
{
	...
	case SIDE_TYPE_STRUCT:
		side_type_struct_attributes(ctx,
					visit_side_sel_pointer(ctx, type->u.side_struct),
					attr, nr_attr);
		break;
	case SIDE_TYPE_ARRAY:
		side_type_array_attributes(ctx,
					visit_side_sel_pointer(ctx, type->u.side_array),
					attr, nr_attr);
		break;
	case SIDE_TYPE_VLA:
		side_type_vla_attributes(ctx,
					visit_side_sel_pointer(ctx, type->u.side_vla),
					attr, nr_attr);
		break;
	case SIDE_TYPE_VARIANT:
		side_type_variant_attributes(ctx,
					visit_side_sel_pointer(ctx, type->u.side_variant),
					attr, nr_attr);
		break;
	case SIDE_TYPE_OPTIONAL:
		side_type_optional_attributes(ctx,
					visit_side_sel_pointer(ctx, type->u.side_optional),
					attr, nr_attr);
		break;
	case SIDE_TYPE_ENUM:
