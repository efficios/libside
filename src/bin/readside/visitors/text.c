			...
			void *ctx)
{
	if (nr_attr && attr) {
		...
		printf_nest(ctx, "attributes: None");
	}
}
...
static void print_null_type_text(const struct side_type_null *type, void *ctx)
{
	(void) type;
	(void) ctx;
}

static void print_bool_type_text(const struct side_type_bool *type, void *ctx)
{
...
static void print_integer_type_text(const struct side_type_integer *type, void *ctx)
{
...
static void print_byte_type_text(const struct side_type_byte *type, void *ctx)
{
	(void) type;
	(void) ctx;
}

static void print_float_type_text(const struct side_type_float *type, void *ctx)
{
...
static void print_string_type_text(const struct side_type_string *type, void *ctx)
{
...
static void print_gather_bool_type_text(const struct side_type_gather_bool *type, void *ctx)
{
	...
	push_nest(ctx); {
		print_bool_type_text(&type->type, ctx);
	} pop_nest(ctx);
}

static void print_gather_integer_type_text(const struct side_type_gather_integer *type, void *ctx)
{
	printf_nest(ctx, "offset: %" PRIu64, type->offset);
	printf_nest(ctx, "offset-bits: %" PRIu16, type->offset_bits);
	printf_nest(ctx, "access-mode: %s",
	...
	printf_nest(ctx, "gather:");
	push_nest(ctx); {
		print_integer_type_text(&type->type, ctx);
	} pop_nest(ctx);
}

static void print_gather_byte_type_text(const struct side_type_gather_byte *type, void *ctx)
{
	printf_nest(ctx, "offset: %" PRIu64, type->offset);
	printf_nest(ctx, "access-mode: %s",
		side_access_mode_to_string(side_enum_get(type->access_mode)));
	printf_nest(ctx, "gather:");
	push_nest(ctx); {
		print_byte_type_text(&type->type, ctx);
	} pop_nest(ctx);
}

static void print_gather_float_type_text(const struct side_type_gather_float *type, void *ctx)
{
	printf_nest(ctx, "offset: %" PRIu64, type->offset);
	printf_nest(ctx, "access-mode: %s",
		side_access_mode_to_string(side_enum_get(type->access_mode)));
	printf_nest(ctx, "gather:");
	push_nest(ctx); {
		print_float_type_text(&type->type, ctx);
	} pop_nest(ctx);
}

static void print_gather_string_type_text(const struct side_type_gather_string *type, void *ctx)
{
	printf_nest(ctx, "offset: %" PRIu64, type->offset);
	printf_nest(ctx, "access-mode: %s",
		side_access_mode_to_string(side_enum_get(type->access_mode)));
	printf_nest(ctx, "gather:");
	push_nest(ctx); {
		print_string_type_text(&type->type, ctx);
	} pop_nest(ctx);
}
...
static void begin_text(struct visitor_context *ctx)
{
	ctx->nesting = 1;
	printf("events:");
}

static void end_text(struct visitor_context *ctx)
{
	ctx->nesting = 0;
}

struct visitor text_visitor = {
	.description = {
		...
		/* Basic types. */
		.null_type_func	   =  print_null_type_text,
		.bool_type_func	   =  print_bool_type_text,
		.integer_type_func =  print_integer_type_text,
		.byte_type_func    =  print_byte_type_text,
		.pointer_type_func =  print_integer_type_text,
		.float_type_func   =  print_float_type_text,
		.string_type_func  =  print_string_type_text,
		...
		/* Basic gather types. */
		.gather_bool_type_func	  =  print_gather_bool_type_text,
		.gather_integer_type_func =  print_gather_integer_type_text,
		.gather_byte_type_func    =  print_gather_byte_type_text,
		.gather_pointer_type_func =  print_gather_integer_type_text,
		.gather_float_type_func   =  print_gather_float_type_text,
		.gather_string_type_func  =  print_gather_string_type_text,
